...
plus direct GET lookups on /api/v1|v2/app/locations/{id}. No login, credentials,
charging actions, account/session mutations, coordinates, or raw response
...
def post_req(path,label,payload_obj):
...
def main():
    ...
    for base in PATHS:
        for location_id in PIN_IDS:
            probes.append(get_req(f'{base}/{location_id}',f'direct_location_{location_id}'))
    report={'schema_version':3,'generated_at':dt.datetime.now(dt.timezone.utc).isoformat(),'host':'cp.evgo.ma','source_pin_ids':PIN_IDS,'policy':{'read_only_hydration':True,'get_single_location_read_only':True,'no_login':True,'no_credentials':True,'no_mutations':True,'no_coordinates_submitted':True,'ids_origin':'anonymous /app/pins public response','raw_response_bodies_persisted':False,'only_whitelisted_public_charging_fields_sampled':True},'post_cases_tested':list(CASES.keys()),'direct_get_ids_tested':PIN_IDS,'probes':probes}
    (OUT/'summary.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
    print(json.dumps([{'method':x.get('method'),'path':x['path'],'case':x['case'],'status':x.get('status'),'counts':x.get('safe_response',{}).get('collection_counts',{}),'keys':x.get('safe_response',{}).get('top_level_keys',[]),'message':x.get('safe_response',{}).get('message')} for x in probes]))
