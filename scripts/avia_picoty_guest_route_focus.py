...
METHODS=['getRegistrationGroups','registerWithoutToken','getTenantIdAsGuest','getTenantIdByRegisterCode','getAzureMapsRegistrationCode','getAppDistributionByTenantId','getNearbyLocationsAsGuest','getMapLocationsAsGuest','getLocationAsGuest','getCposAsGuest','getTenantFiles','getLocationTariffsAsGuest','simulateLocationPricingAsGuest']
...
ROUTE=re.compile(r'''(?x)(?:https?://[^\s"'<>]{3,220}|["']([^"']*(?:v1/|tenant|register|registration|distribution|location|tariff|map|cpos|files)[^"']{0,220})["'])''',re.I)
...
def main():
    out={'schemaVersion':'1.1.1','generatedAt':datetime.now(timezone.utc).isoformat(),'methods':{m:[] for m in METHODS}}
    for p in INPUTS:
        ...
        for path,s in walk(data):
            ...
            for method in METHODS:
                ...
                while True:
                    ...
                    if i<0: break
                    a=max(0,i-4500); b=min(len(s),i+len(method)+9000)
                    ...
                    out['methods'][method].append({'source':str(p),'jsonPath':path,'context':context,'routeLike':routes[:100]})
                    start=i+len(method)
                    if len(out['methods'][method])>=20: break
    for m,items in out['methods'].items():
        seen=set(); clean=[]
        for x in items:
            key=x['context']
            if key not in seen: seen.add(key); clean.append(x)
        out['methods'][m]=clean[:20]
