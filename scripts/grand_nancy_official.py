...
def req_regex(t,label,p):
 if not re.search(p,norm(t),re.I):raise RuntimeError('Grand Nancy evidence missing: '+label)
def main():
 ...
 req(lt,'plus de 70 points de recharge','21 sites','MODULO','Liste des parkings équipés')
 req_regex(lt,'1er avril 2023',r'depuis\s+le\s+1\s*(?:er)?\s+avril\s+2023')
