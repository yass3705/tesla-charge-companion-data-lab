...
def main():
    ...
    hs,hraw,hfinal=fetch(HOME); rs,rraw,rfinal=fetch(RULES); ts,traw,tfinal=fetch(TE28); ss,sraw,sfinal=fetch(SIEIL37); cs,craw,cfinal=fetch(SDE18); bs,braw,bfinal=fetch(SIDELC41)
    if min(hs,rs,ts,ss,cs,bs)!=200: raise RuntimeError(f'HTTP failure home={hs} rules={rs} te28={ts} sieil37={ss} sde18={cs} sidelc41={bs}')
    h=hraw.decode('utf-8',errors='replace'); t=traw.decode('utf-8',errors='replace'); s=sraw.decode('utf-8',errors='replace'); c=craw.decode('utf-8',errors='replace'); b=braw.decode('utf-8',errors='replace')
    ...
    require(b,'Bornes de recharge','SIDELC','Loir et Cher','Modulo')
    try:
    ...
    payload={
      ...
      'centreValDeLoireEvidence':{
        ...
        'loirEtCher':{'authority':'SIDELC - Syndicat intercommunal de distribution dՎnergie de Loir-et-Cher','operatorConfirmed':True,'evidence':'Agglopolys public open-data catalogue explicitly identifies SIDELC Loir-et-Cher chargers as Modulo'}
      },
      'tccDecision':{
      ...
      'sourceEvidence':{
        ...
        'sidelc41OpenDataUrl':bfinal,'sidelc41OpenDataHttpStatus':bs,'sidelc41OpenDataSha256':hashlib.sha256(braw).hexdigest()
      },
    ...
    (out/'SUMMARY.md').write_text('# Modulo Energies / Centre-Val de Loire\n\nOperator validated across Cher, Eure-et-Loir, Indre-et-Loire and Loir-et-Cher. Current public homepage confirms free subscription from 0.40 EUR/kWh and ad-hoc from 0.52 EUR/kWh, but these are explicitly from-prices; keep Modulo reference-only until the exact local/station tariff is resolved.\n')
