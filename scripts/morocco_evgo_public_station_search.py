...
Uses only already-confirmed public mobile search routes and public place-name
queries. No login, credentials, mutation, charging/payment action or raw response
persistence. The report keeps only charging-infrastructure fields plus harmless
...
QUERIES=["Marjane Mohammedia","Marjane","Mohammedia"]
...
def main():
 ...
 result={"schema_version":2,"generated_at":dt.datetime.now(dt.timezone.utc).isoformat(),"host":HOST,"queries":QUERIES,"policy":{"read_only":True,"no_login":True,"no_mutations":True,"no_credentials":True,"raw_response_bodies_persisted":False},"probes":probes}
