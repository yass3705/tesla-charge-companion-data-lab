...
def sanitize(text: str) -> str:
    return JWT_RE.sub("<redacted-jwt>", SECRETISH.sub(lambda m: m.group(1) + "=<redacted>", text))[:2500]
...
def request(method: str, url: str, params: dict[str, str] | None = None) -> dict:
    ...
    try:
        with urllib.request.urlopen(req, timeout=20) as r:
            ...
            return {"method":method,"url":url,"status":int(r.status),"contentType":r.headers.get("content-type"),"wwwAuthenticate":r.headers.get("www-authenticate"),"allow":r.headers.get("allow"),"body":sanitize(body)}
    except urllib.error.HTTPError as e:
        body=e.read(8192).decode("utf-8","replace")
        return {"method":method,"url":url,"status":int(e.code),"contentType":e.headers.get("content-type") if e.headers else None,"wwwAuthenticate":e.headers.get("www-authenticate") if e.headers else None,"allow":e.headers.get("allow") if e.headers else None,"body":sanitize(body)}
    ...
    for base in BASES:
        for prefix in PREFIXES:
            for path in PATHS:
                ...
                seen.add(url); results.append(request("GET",url))
