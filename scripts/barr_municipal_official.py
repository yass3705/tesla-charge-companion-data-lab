...
def main():
 ...
 must_regex(t,'surcharge after 2h',r'(?:au[- ]?dela|apres).{0,80}2\s*h.{0,100}(?:branche|branchement|connexion)')
 must_regex(t,'2 EUR per started hour',r'2\s*€?\s*(?:ttc\s*)?(?:par\s*)?heure.{0,40}entam')
 must_regex(t,'CCPB B22/2024 harmonisation reference',r'b\s*22\s*/\s*2024.{0,160}ccpb|ccpb.{0,160}b\s*22\s*/\s*2024')
