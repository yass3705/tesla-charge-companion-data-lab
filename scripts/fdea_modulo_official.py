...
def main():
    ...
    require(mt,'SERVICE DE RECHARGE MODULO','Prise type 2','2€ / heure','Prise CHAdeMO / Combo','3€ / heure','décompté à la minute','30 % plus chère','Frais de recharge minimum : 0,50 €','Réservation de borne : 0,01 € / minute','Carte d’abonnement : 10€')
    ...
    (out/'fdea_modulo_official_grandest.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n');(out/'SUMMARY.md').write_text('# FDEA Ardennes / Modulo\n\nOfficial FDEA evidence validates the rural Ardennes IRVE network outside Ardenne Métropole and current Modulo operation. Subscriber tariff: AC Type 2 2 EUR/hour and DC CHAdeMO/Combo 3 EUR/hour, prorated by minute. Ad-hoc is 30% higher with a 0.50 EUR minimum. Reservation is 0.01 EUR/min up to 30 minutes; a new RFID card request is 10 EUR.\n')
