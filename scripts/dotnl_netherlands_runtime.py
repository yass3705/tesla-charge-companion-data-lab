...
DAY_INDEX={name:i+1 for i,name in enumerate(DAYS)}  # JS Date convention is adapted below
...
def current_element(el,today):
    ...
    s=date_of(rr.get('start_date')); e=date_of(rr.get('end_date'))
...
def compile_tariff(t,today):
    ...
    # FLAT is only exact in TCC when it is a single unrestricted connection fee.
    flat_components=[]
    for el in current:
        rr=el.get('restrictions') or {}
        for pc in el.get('priceComponents') or []:
            if str(pc.get('type') or '').upper()=='FLAT':
                if any(rr.get(k) not in (None,[],{},'') for k in ('start_time','end_time','day_of_week')):
                    return None,'restricted_flat'
    ...
    for day_name in DAYS:
        js_day=JS_DAY[day_name]
        for a,b in zip(bounds,bounds[1:]):
            if b<=a: continue
            probe=a+(b-a)/2
            vals={}
            for dim in ('ENERGY','TIME','PARKING_TIME'):
                pc=component_for_dimension(current,dim,day_name,probe)
                vals[dim]=component_gross(pc) if pc else 0.0
            # No costs at all is still a valid free interval.
            billing='kwh' if vals['ENERGY']>0 else ('minute' if vals['TIME']>0 else 'kwh')
            rows.append([
                'timeWindow',hhmm(a),'24:00' if b==1440 else hhmm(b),billing,(t.get('currency') or 'EUR').upper(),
                round(vals['ENERGY'],6),round(vals['TIME']/60,8),round(flat,6),round(vals['PARKING_TIME']/60,8),0,0,[js_day]
            ])
    # Merge identical windows across weekdays.
    merged={}
    for r in rows:
    ...
    out.sort(key=lambda r:(r[1],r[2],r[11]))
    return out,None

def tariff_rank(t):
    typ=str(t.get('type') or '').upper()
    return 0 if typ=='AD_HOC_PAYMENT' else (1 if typ=='REGULAR' else 2)

def choose_tariff(keys,tariffs,today,stats):
    candidates=[]
    for key in keys:
        t=tariffs.get(key)
        if not t: continue
        rules,reason=compile_tariff(t,today)
        if reason:
            stats['unsupportedReasons'][reason]+=1
            continue
        candidates.append((tariff_rank(t),key,t,rules))
    if not candidates: return None,None,'none_exact'
    best_rank=min(x[0] for x in candidates)
    candidates=[x for x in candidates if x[0]==best_rank]
    sigs={json.dumps(x[3],separators=(',',':')) for x in candidates}
    if len(sigs)>1:
        stats['ambiguousTariffConnectors']+=1
...
def kind(conn):
    ...
    if pt=='DC' or 'CHADEMO' in st or 'COMBO' in st or 'CCS' in st: return 'DC'
    return 'AC'
...
def tile_id(lat,lon,size=.5):
    a=math.floor(lat/size)*size; b=math.floor(lon/size)*size
    fmt=lambda x:str(round(x*2)).replace('-','m')
...
def main():
    ...
    tariffs=data.get('tariffs') or {}; stations=data.get('stations') or []
    generated=str(data.get('generatedAt') or dt.datetime.now(dt.timezone.utc).isoformat())
    today=date_of(generated) or dt.datetime.now(dt.timezone.utc).date()
    stats={'connectors':0,'exactPricedConnectors':0,'unpricedConnectors':0,'ambiguousTariffConnectors':0,'unsupportedReasons':collections.Counter(),'configs':0,'pricedConfigs':0}
    rows=[]
    for st in stations:
        groups={}
        for evse in st.get('evses') or []:
            ...
            for c in evse.get('connectors') or []:
                stats['connectors']+=1
                p=fnum(c.get('powerKw'))
                if not p or p<=0: continue
                tkey,rules,err=choose_tariff(c.get('tariffKeys') or [],tariffs,today,stats)
                priced=rules is not None
                if priced: stats['exactPricedConnectors']+=1
                else: stats['unpricedConnectors']+=1
                sig=json.dumps(rules,separators=(',',':')) if rules is not None else 'UNPRICED'
                gkey=(kind(c),round(p,1),sig)
                g=groups.setdefault(gkey,{'kind':gkey[0],'power':gkey[1],'rules':rules,'tariffKey':tkey,'evses':set(),'count':0})
                g['count']+=1
                if uid: g['evses'].add(uid)
        configs=[]
        for i,g in enumerate(sorted(groups.values(),key=lambda x:(x['kind'],x['power'],x['tariffKey'] or ''))):
            stalls=len(g['evses']) or g['count']; priced=g['rules'] is not None
            label=('DOT-NL public' if priced else 'Tarif DOT-NL non calculable')+f" · {g['kind']} {g['power']:g} kW"
            cid=f"dotnl-{i}-{g['kind'].lower()}-{str(g['power']).replace('.','_')}"
            configs.append([cid,label,g['kind'],g['power'],stalls,g['rules'] or []])
            stats['configs']+=1
            if priced: stats['pricedConfigs']+=1
        if not configs: continue
        co=st.get('coordinates') or {}; lat=fnum(co.get('latitude')); lon=fnum(co.get('longitude'))
        if lat is None or lon is None: continue
        name=str(st.get('name') or '').strip() or f"Borne {st.get('stationId')}"
        address=', '.join(x for x in [str(st.get('address') or '').strip(),str(st.get('postalCode') or '').strip(),str(st.get('city') or '').strip()] if x)
        operator=str(st.get('operatorName') or '').strip() or str(st.get('partyId') or 'DOT-NL')
    ...
    for (tid,a,b),arr in sorted(tiles.items()):
        arr.sort(key=lambda r:str(r[0])); path=args.out_dir/f'{tid}.json.gz'; raw_n,gz_n=gz_write(path,arr)
        manifest_tiles.append({'id':tid,'file':path.name,'minLat':a,'maxLat':a+.5,'minLon':b,'maxLon':b+.5,'count':len(arr),'bytes':gz_n,'sha256':hashlib.sha256(path.read_bytes()).hexdigest()})
    rows.sort(key=lambda r:str(r[0])); raw_all,gz_all=gz_write(args.out_dir/'all.json.gz',rows)
    manifest={'schemaVersion':1,'dataset':'netherlands-non-tesla-runtime-test','generatedAt':generated,'effectiveTariffDate':today.isoformat(),'stationCount':len(rows),'configurationCount':stats['configs'],'pricedConfigurationCount':stats['pricedConfigs'],'tileSizeDegrees':.5,'tileCount':len(manifest_tiles),'allFile':'all.json.gz','allBytes':gz_all,'tiles':manifest_tiles,'scope':{'countryCode':'NL','teslaExcluded':True,'strictTariffCompiler':True,'publishedToTcc':False}}
    (args.out_dir/'manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    report={'dataset':'dotnl-netherlands-runtime-report','generatedAt':generated,'effectiveTariffDate':today.isoformat(),'stationCount':len(rows),'tileCount':len(manifest_tiles),'allCompressedBytes':gz_all,'allUncompressedBytes':raw_all,'metrics':{**{k:v for k,v in stats.items() if not isinstance(v,collections.Counter)},'unsupportedReasons':dict(stats['unsupportedReasons'].most_common())},'coveragePct':{'exactTariffConnectors':round(100*stats['exactPricedConnectors']/stats['connectors'],3) if stats['connectors'] else 0,'pricedConfigs':round(100*stats['pricedConfigs']/stats['configs'],3) if stats['configs'] else 0},'publishedToTcc':False}
