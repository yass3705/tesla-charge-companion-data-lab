...
def parse_exact_tariff(tariff):
 ...
 if time_min:
  match=time_min[0];price=amount(match.group(1));occupied_cues=(components.get('continuesWhilePluggedIn') or re.search(r"tarification continue tant|facturation continue tant|billing continues as long|pricing continues as long|charging continues as long|charge applies as long",text,re.I) or re.search(r"temps de branchement|dur[eé]e de branchement|temps de parking|connection time|duration of (?:the )?connection|parking time|time the vehicle is plugged",text,re.I));exact['time']={'amount':price,'billing':'started_minute','appliesTo':'occupied' if occupied_cues else 'charge'};explained.append(price)
 if time_hour:
