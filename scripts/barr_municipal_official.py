...
def main():
 ...
 cs,craw,cfinal=fetch(CITY);ps,praw,pfinal=fetch(CCPB);pt=norm(txt(praw))
 if cs!=200 or ps!=200 or not craw.startswith(b'%PDF') or not praw.startswith(b'%PDF'):raise RuntimeError(f'official PDF failure city={cs} ccpb={ps}')
 # Pays de Barr official council record is the durable machine-readable tariff witness.
 must(pt,'Barr location',r'jardin\s+des\s+sports.{0,120}barr|barr.{0,120}jardin\s+des\s+sports')
