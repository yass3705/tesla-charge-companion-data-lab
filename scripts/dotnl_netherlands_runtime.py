...
def choose_tariff(keys,tariffs,today,stats,compile_cache,selection_cache):
    ...
    if not candidates:
        result=(None,None,'none_exact',dict(reasons)); selection_cache[selector]=result
    else:
        ...
        if len(sigs)>1: result=(None,None,'ambiguous',dict(reasons)); selection_cache[selector]=result
        else:
            x=candidates[0]; result=(x[1],x[3],None,dict(reasons)); selection_cache[selector]=result
    tkey,rules,err,reasons=result
    for reason,count in reasons.items(): stats['unsupportedReasons'][reason]+=count
    if err=='ambiguous': stats['ambiguousTariffConnectors']+=1
    return tkey,rules,err
