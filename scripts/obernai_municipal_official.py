...
DOC='https://app.obernai.fr/view_document.php?id=2371'
def fetch(u):
...
def main():
 ...
 ds,dr,df,dct=fetch(DOC);dt=text(dr,dct)
 req(dt,'10 emplacements de charge','0,30 € / KWh','3e et 4e heures','0,02','A partir de la 5e heure','0,10','Freshmile')
 p={'schemaVersion':'1.0.0','dataset':'obernai-municipal-official-grandest','generatedAt':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'operator':'Ville d’Obernai - réseau municipal','serviceOperator':'Freshmile','country':'FR','region':'Grand Est','department':'Bas-Rhin','classification':{'municipalPublicNetwork':True,'directPublishedTariff':True,'energyBased':True,'parkingTimeBased':True,'directTariffClassable':True},'network':{'publishedChargeSpaces':10,'powerKw':22,'scope':'municipal charge spaces covered by official city publication'},'operatorDirect':{'energyEurPerKwh':0.30,'parking':{'firstTwoHoursEur':0.0,'thirdFourthHoursEurPerMinute':0.02,'fromFifthHourEurPerMinute':0.10}},'tccDecision':{'operatorValidated':True,'directTariffClassable':True,'energyAndParkingMustRemainSeparateComponents':True,'stationTestsDeferred':True,'roamingSeparate':True},'sourceEvidence':{'officialOnly':True,'cityPublicationUrl':df,'cityPublicationHttpStatus':ds,'cityPublicationSha256':hashlib.sha256(dr).hexdigest()},'publicationStatus':'validated_candidate'}
