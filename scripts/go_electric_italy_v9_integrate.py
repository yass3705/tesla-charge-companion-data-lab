...
def normalized_direct_from_offer(offer: dict) -> dict:
    ...
    if currency != "EUR":
        raise ValueError(f"{offer.get('punEvseId')}: currency must be EUR, got {currency!r}")
    components = offer.get("priceComponents")
    if not isinstance(components, list) or not components:
        raise ValueError(f"{offer.get('punEvseId')}: priceComponents missing")
    ...
    for component in components:
        if not isinstance(component, dict):
            raise ValueError(f"{offer.get('punEvseId')}: invalid component")
        kind = str(component.get("sourceType") or "")
        if kind not in ALLOWED_COMPONENTS:
            raise ValueError(f"{offer.get('punEvseId')}: unknown component {kind!r}")
        if kind in seen:
            raise ValueError(f"{offer.get('punEvseId')}: duplicate component {kind!r}")
        seen.add(kind)
        amount = component.get("amount")
        if isinstance(amount, bool) or not isinstance(amount, (int, float)) or amount < 0:
            raise ValueError(f"{offer.get('punEvseId')}: invalid amount for {kind}")
        normalized = {
            "type": kind,
            "amount": amount,
            "unit": component.get("sourceUnit"),
        }
    ...
    if energy is None:
        raise ValueError(f"{offer.get('punEvseId')}: energy component required")
    ...
    if provenance.get("exactPhysicalIdentity") is not True:
        raise ValueError(f"{offer.get('punEvseId')}: exactPhysicalIdentity required")
    if qa.get("semanticValidated") is not True or qa.get("powerCompatible") is not True:
        raise ValueError(f"{offer.get('punEvseId')}: semantic/power QA required")
...
def main() -> None:
    ...
    if len(offers) != 2214:
        raise SystemExit(f"expected 2214 ready offers, got {len(offers)}")

    qa_summary = semantic_qa.get("summary") or {}
    if qa_summary.get("readyEvseOffers") != 2214:
        raise SystemExit("semantic QA readyEvseOffers mismatch")
    input_baseline = semantic_qa.get("inputBaseline") or {}
    if input_baseline.get("stationCount") != 1136 or input_baseline.get("evseCount") != 2413:
        raise SystemExit("semantic QA Go Electric baseline mismatch")
    ...
    if station_quarantine != 79 or connector_exclusion != 48:
        raise SystemExit(
    ...
    if missing:
        raise SystemExit(f"{len(missing)} accepted Go Electric EVSE IDs absent from candidate; first={missing[:5]}")
    ...
    for e in evses:
        ...
        if existing_rankable:
            ...
            if existing_operator != GO_ELECTRIC_OPERATOR:
                conflicts.append(
                    {"evseId": evse_id, "existingOperator": existing_operator or "UNKNOWN"}
                )
                continue
    ...
    if conflicts:
        raise SystemExit(
            f"{len(conflicts)} conflicting rankable direct tariffs; first={conflicts[:5]}"
        )
    ...
    if final_direct_by_operator.get(GO_ELECTRIC_OPERATOR, 0) != 2214:
        raise SystemExit(
    ...
    if len(target_by_station) != 1136 or sum(target_by_station.values()) != 2413:
        raise SystemExit(
            f"candidate Go Electric physical inventory drift: {len(target_by_station)} stations/"
            f"{sum(target_by_station.values())} EVSE"
        )
    ...
    report = {
        ...
        "input": {
            ...
            "goElectricPhysicalStations": len(target_by_station),
            "goElectricPhysicalEvse": sum(target_by_station.values()),
        ...
        "integration": {
            ...
            "stationCoverage": {
                ...
                "completeStations": complete_stations,
        ...
        "gates": {
            "semanticReadyOfferCount2214": len(offers) == 2214,
            "allAcceptedOffersMatchedCandidate": not missing,
            "noDirectConflicts": not conflicts,
            "goElectricDirectCount2214": final_direct_by_operator.get(GO_ELECTRIC_OPERATOR, 0) == 2214,
