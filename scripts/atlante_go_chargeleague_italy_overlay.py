...
def main():
    ...
    # Fail closed if a named partner unexpectedly maps to multiple PUN party IDs.
    ...
    Path(a.out).parent.mkdir(parents=True,exist_ok=True); s=json.dumps(payload,ensure_ascii=False,indent=2)+'\n'; Path(a.out).write_bytes(gzip.compress(s.encode(),compresslevel=9,mtime=0)); Path(a.report).write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2))
