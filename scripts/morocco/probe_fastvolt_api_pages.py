...
try:
    ...
    with urlopen(req,timeout=20) as r:
        b=r.read(MAX_BYTES+1); report['status']=getattr(r,'status',200); report['content_type']=r.headers.get('Content-Type'); report['bytes_read']=min(len(b),MAX_BYTES); report['truncated']=len(b)>MAX_BYTES
        b=b[:MAX_BYTES]
        try:
            ...
            if isinstance(data,list):
                ...
                keys=set()
                for x in data[:100]:
                    if isinstance(x,dict): keys.update(str(k) for k in x.keys())
                report['item_keys']=sorted(keys)[:120]
            elif isinstance(data,dict):
                report['top_level_keys']=sorted(str(k) for k in data.keys())[:120]
                for k,v in data.items():
                    if isinstance(v,list):
                        report.setdefault('list_fields',[]).append({'field':str(k),'count':len(v),'item_keys':sorted({str(kk) for x in v[:100] if isinstance(x,dict) for kk in x.keys()})[:120]})
        except Exception as ex:
            report['json']=False; report['parse_error']=type(ex).__name__
except HTTPError as ex:
    report['status']=ex.code; report['content_type']=ex.headers.get('Content-Type'); report['http_error']=True
except Exception as ex:
