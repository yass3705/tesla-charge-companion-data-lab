...
BASES=['https://pdefweushaapiam01.azure-api.net','https://adefweuappbckfa01.azurewebsites.net','https://api.deftpower.com']
...
def request(method,url,params=None):
    ...
    try:
        with urllib.request.urlopen(req,timeout=20) as r:return {'method':method,'url':url,'status':int(r.status),'contentType':r.headers.get('content-type'),'wwwAuthenticate':r.headers.get('www-authenticate'),'allow':r.headers.get('allow'),'body':sanitize(r.read(8192).decode('utf-8','replace'))}
    except urllib.error.HTTPError as e:return {'method':method,'url':url,'status':int(e.code),'contentType':e.headers.get('content-type') if e.headers else None,'wwwAuthenticate':e.headers.get('www-authenticate') if e.headers else None,'allow':e.headers.get('allow') if e.headers else None,'body':sanitize(e.read(8192).decode('utf-8','replace'))}
    except Exception as e:return {'method':method,'url':url,'status':None,'error':type(e).__name__,'message':sanitize(str(e))}
def main():
    results=[];seen=set()
    for base in BASES:
        for prefix in PREFIXES:
            for path in PATHS:
                ...
                if url not in seen:seen.add(url);results.append(request('GET',url))
            results.append(request('GET',base.rstrip('/')+prefix+'/tenants/nonexistent/map-locations',{'latLongBottomLeft':'41,-6','latLongTopRight':'52,10'}))
            results.append(request('GET',base.rstrip('/')+prefix+'/tenants/nonexistent/nearby-locations',{'latitude':'48.8','longitude':'2.3'}))
            results.append(request('OPTIONS',base.rstrip('/')+prefix+'/tenants/nonexistent/map-locations'))
    payload={'generatedAt':datetime.now(timezone.utc).isoformat(),'credentialsSent':False,'writeMethodsUsed':False,'bases':BASES,'prefixes':PREFIXES,'results':results}
    OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    summary={}
    for x in results:summary[str(x.get('status'))]=summary.get(str(x.get('status')),0)+1
    print(json.dumps({'count':len(results),'statusCounts':summary},ensure_ascii=False,indent=2))
    for x in results:
        if x.get('status') not in (404,None):print(json.dumps({'method':x['method'],'url':x['url'],'status':x.get('status'),'body':x.get('body','')[:500]},ensure_ascii=False))
