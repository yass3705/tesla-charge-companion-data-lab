...
def get(url,timeout=90):
    q=urllib.request.Request(url,headers={'User-Agent':UA,'Referer':'https://apkpure.net/'})
...
def discover(blob):
    scored={}; hitfiles=[]; diagnostics=[]
    for name,d in payloads(blob):
      ss=list(astrings(d)); joined='\n'.join(ss)
      interesting=('azure-api.net' in joined or 'Ocp-Apim' in joined or 'subscription' in joined.lower() or 'atlante' in joined.lower())
      if 'azure-api.net' in joined or 'Ocp-Apim-Subscription-Key' in joined: hitfiles.append(name)
      if interesting:
       diagnostics.append({'file':name,'hasApiHost':'azure-api.net' in joined,'hasApimHeader':'Ocp-Apim-Subscription-Key' in joined,'stringCount':len(ss)})
      for s in ss:
       low=s.lower(); bonus=30 if ('ocp-apim' in low or 'subscription' in low) else 0
       bonus+=20 if 'azure-api.net' in low else 0
       # Azure APIM keys are commonly 32-char opaque client constants; allow base64url/alnum forms.
       for c in re.findall(r'(?<![A-Za-z0-9_-])[A-Za-z0-9_-]{28,48}(?![A-Za-z0-9_-])',s):
        ...
        score=bonus+(10 if len(c)==32 else 0)+(5 if re.fullmatch(r'[0-9a-fA-F]{32}',c) else 0)
        if interesting:score+=10
        scored[c]=max(scored.get(c,-1),score)
    return [x for x,_ in sorted(scored.items(),key=lambda kv:(-kv[1],kv[0]))],sorted(set(hitfiles)),diagnostics[:80]
...
def save(rep):OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(rep,ensure_ascii=False,indent=2)+'\n')

def main():
 rep={'source':'public myAtlante Android package','clientCredentialRecovered':False,'clientCredentialPersisted':False,'attempts':[],'security':SEC}
 working=mp=None;version=None
 for ver,url in XAPKS:
  try:blob=get(url)
  except Exception as e:
   rep['attempts'].append({'version':ver,'download':False,'errorType':type(e).__name__});continue
  try:cands,hits,diag=discover(blob)
  except Exception as e:
   rep['attempts'].append({'version':ver,'download':True,'scan':False,'errorType':type(e).__name__});continue
  attempt={'version':ver,'download':True,'candidateCount':len(cands),'configHitFiles':hits,'diagnostics':diag,'probed':0}
  for c in cands[:600]:
   attempt['probed']+=1;print(f'::add-mask::{c}')
   try:
    st,p=api(MAP,c,ver)
    if st==200 and isinstance(p,dict) and isinstance(p.get('locations'),list):working,mp,version=c,p,ver;break
   except Exception:pass
  rep['attempts'].append(attempt)
  if working:break
 if not working:
  save(rep);raise SystemExit('No public-client API credential validated; sanitized report saved')
 rep['clientCredentialRecovered']=True;rep['testedVersion']=version
 locs=[l for l in mp.get('locations',[]) if str(l.get('countryCode','')).upper()=='IT' and str(l.get('partyId','')).upper()=='ATE']
 rep['italyAtlanteMapLocations']=len(locs);samples=[]
 for l in locs[:80]:
  lid=str(l.get('id') or '')
  if not lid:continue
  ...
  tariffs=tp if isinstance(tp,list) else (tp.get('tariffs',[]) if isinstance(tp,dict) else [])
  rows=[]
  for t in tariffs:
